...
void set<Kty>::eraseDegreeOne(TreeNode<Kty> *node)
{
  ...
  if (node->color == Red)
  { // Simple Case 1 -> Red node with two leaves
    if (node == node->parent->right)
    {
  ...
  else if (onlyChild->color == Red)
  { // Simple Case 2 -> node is Black, onlyChild is Red
    node->color = Black;
  ...
  { // Complicate Casse: Black Node with two Nil Black Nodes
