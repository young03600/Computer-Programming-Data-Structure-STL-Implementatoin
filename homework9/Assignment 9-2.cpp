...
#include "HugeInteger 9-2 - 1083520 -.h"
...
int main()
{
   ...
   HugeInteger< vector< int > > n17[ numTestCases ];
   HugeInteger< vector< int > > n18[ numTestCases ];
   HugeInteger< vector< unsigned int > > n19[ numTestCases ];
   HugeInteger< vector< unsigned int > > n20[ numTestCases ];
   HugeInteger< vector< short int > > n21[ numTestCases ];
   HugeInteger< vector< short int > > n22[ numTestCases ];
   HugeInteger< vector< unsigned short int > > n23[ numTestCases ];
   HugeInteger< vector< unsigned short int > > n24[ numTestCases ];
...
int enterChoice()
{
   cout << "Enter your choice\n"
      << " 1 - test deque< int >\n"
      << " 2 - test deque< unsigned int >\n"
      << " 3 - test deque< short int >\n"
      << " 4 - test deque< unsigned short int >\n"
      << " 5 - test list< int >\n"
      << " 6 - test list< unsigned int >\n"
      << " 7 - test list< short int >\n"
      << " 8 - test list< unsigned short int >\n"
      << " 9 - test vector< int >\n"
      << " 10 - test vector< unsigned int >\n"
      << " 11 - test vector< short int >\n"
      << " 12 - test vector< unsigned short int >\n";
...
void input( HugeInteger< T > n1[], HugeInteger< T > n2[] )
{
   ...
   for( int i = 0; i < numTestCases; i++ )
   {
      ...
      while( ( digit = inFile.get() ) != '\n' )
         buffer1.insert( buffer1.cbegin(), digit - '0' );

      while( ( digit = inFile.get() ) != '\n' )
         buffer2.insert( buffer2.cbegin(), digit - '0' );
      inFile.get();
