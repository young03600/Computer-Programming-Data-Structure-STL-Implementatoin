...
public:
   ...
   list& operator=(const list &right)
   {
	   if (this != &right) {
		   ...
		   int diff = right.size() > size() ? right.size() - size() : size() - right.size();
		   while (diff--)
			   erase(end()->prev);
   ...
   {
	   iterator _where = iterator(where);

	   nodePtr newNode = new node;
	   newNode->myVal = val;

	   newNode->next = _where;
	   newNode->prev = _where->prev;
	   _where->prev->next = newNode;
	   _where->prev = newNode;
   ...
   iterator erase( const_iterator where )
   {
	   ...
	   _where->prev->next = _where->next;
	   _where->next->prev = _where->prev;
	   delete _where;
...
bool operator==( const list< Ty > &left, const list< Ty > &right )
{
	...
	typename list<Ty>::const_iterator start1 = left.begin();
	typename list<Ty>::const_iterator start2 = right.begin();
	typename list<Ty>::const_iterator end1 = left.end();
	typename list<Ty>::const_iterator end2 = right.end();
